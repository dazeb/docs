...
The `postal` part of the DKIM record name is the selector. If you wish to use a different selector, set `dns.dkim_identifier` in `postal.yml` before publishing the record.
...
Each route has a unique forwarding address on this domain, such as `{token}@routes.postal.example.com`. See [Routing incoming e-mail](/features/routing-incoming-email) for details.
